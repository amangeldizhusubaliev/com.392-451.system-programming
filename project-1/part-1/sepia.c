...
int main(int argc, char *argv[])
{
    ...
    /* Main Image Processing Loop */
    {
        ...
        for (size_t position = 0; position < channels_count; position += step) {
            ...
            static const float Sepia_CoefficientsLOL[]  = {
            ...
            float Sepia_Coefficients[48] __attribute__((aligned(64)));
            for(int i = 0; i < 48; i ++) Sepia_Coefficients[i] = Sepia_CoefficientsLOL[i];
            ...
            */
            ...
            __asm__ __volatile__ (
                ...
                "vmovaps %%zmm3, %%zmm5\n\t"
                "vmovaps %%zmm3, %%zmm6\n\t"
                "vpermilps $0xC1, %%zmm4, %%zmm4\n\t"
                "vpermilps $0xD5, %%zmm5, %%zmm5\n\t"
                "vpermilps $0xEA, %%zmm6, %%zmm6\n\t"
                "vmulps %%zmm0, %%zmm4, %%zmm3\n\t"
                "vfmadd231ps %%zmm5, %%zmm1, %%zmm3\n\t"
                "vfmadd231ps %%zmm6, %%zmm2, %%zmm3\n\t"
                "vcvtps2dq %%zmm3, %%zmm3\n\t"
                "vpmovusdb %%zmm3, (%0,%1)\n\t"
            ::
                "b"(pixels), "c"(position), "D"(Sepia_Coefficients), "S"(Sepia_Coefficients + 16), "d"(Sepia_Coefficients + 32)
            :
