    ...
    /* Main Image Processing Loop */
    {
        ...
        for (size_t position = 0; position < channels_count; position += step) {
            ...
            */
            ...
            __m512 br = _mm512_broadcastss_ps(_mm_set_ps1(brightness));
            __m512 ct = _mm512_broadcastss_ps(_mm_set_ps1(contrast));
            __m512i px = _mm512_cvtepu8_epi32(_mm_load_si128((__m128i *) &pixels[position]));
            __m512 fpx = _mm512_cvtepi32_ps(px);
            fpx = _mm512_fmadd_ps(ct, fpx, br);
            px = _mm512_cvtps_epi32(fpx);
            _mm512_mask_cvtusepi32_storeu_epi8(&pixels[position], 0xffff, px);
