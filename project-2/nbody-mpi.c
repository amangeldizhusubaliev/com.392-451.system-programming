...
int main(int argc, char **argv)
{
    ...
    size_t bodies_per_process = (body_count + world_size - 1) / world_size;
    ...
    for (size_t k = 0, next_acceleration = 0; k < iterations; ++k) {
        ...
        for (size_t i = begin, sub_i = 0; i <= end; ++i, ++sub_i) {
            integrate(&sub_bodies[sub_i], delta_time);
        }

        MPI_Allgather(sub_bodies, bodies_per_process, mpi_dt_body, bodies, bodies_per_process, mpi_dt_body, MPI_COMM_WORLD);
